...
exports.all = (req, res, next) => {
  all(Patient)
    .then((all) =>
      res.status(200).json(all.map((patient) => modelToDto(patient)))
    )
    .catch((err) => next(err));
};

exports.model = (req, res, next) => {
  console.log(predict(req.body.text));
...
exports.get = (req, res, next) => {
  one(Patient, req)
    .then((patient) => {
      if (patient)
        children(Appointment, { patientId: patient._id }, req).then(
          (appointments) => {
            res
              .status(200)
              .json(Object.assign({}, modelToDto(patient), { appointments }));
          }
        );
      ...
      if (patient)
        children(Appointment, { patientId: patient._id }, req).then(
          async (appointments) => {
            ...
            // delete client
            deleteEntity(Patient, req, res, next).then((_) => {
              res.sendStatus(200);
            });
          }
        );
